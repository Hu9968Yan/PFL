...
# 加载公共测试集
public_test_dataset = ImageFolder(r'D:\HuYan\BT\Testing', transform=data_transforms)
public_test_loader = DataLoader(public_test_dataset, batch_size=16, shuffle=False)

# 测试函数
def test(model, test_loader):
    model.eval()
    target_num = torch.zeros((1, 4))  # 9 是 class_num
...
# 加载模型并测试
save_dir = r'D:\HuYan\BT\savemodel\rate\0.0005'
...
for i in range(1, 5):
    ...
    # model = timm.create_model('mobilenetv2_100', pretrained=False, num_classes=9)
    # model = timm.create_model('convnext_base', pretrained=False, num_classes=9)
    # model = timm.create_model('edgenext_base.in21k_ft_in1k', pretrained=False, num_classes=9)
    ...
    # 进行测试
    test_accuracy, recall, precision, F1, target_num = test(model, public_test_loader)
